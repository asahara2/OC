...
export const DISPLAY_SITE_NAME = "おっぱい共同体";
...
export function publicIdentity(settings: PublicSettings) {
  const name = !settings.siteName || settings.siteName === "Organization" ? INITIAL_SITE_NAME : settings.siteName;
  return {
    name,
    displayName: name === INITIAL_SITE_NAME ? DISPLAY_SITE_NAME : name,
