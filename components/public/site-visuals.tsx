...
export function SiteVisuals({ settings }: { settings: PublicSettings }) {
  ...
  useEffect(() => {
    ...
    if (!client) return;
    const channel = client.channel("live-site-visuals").on("postgres_changes", { event: "*", schema: "public", table: "site_settings" }, async () => {
      const response = await fetch("/api/site-settings", { cache: "no-store" });
      if (response.ok) { const payload = await response.json() as { data?: PublicSettings }; if (payload.data) setCurrent(payload.data); }
    }).subscribe();
